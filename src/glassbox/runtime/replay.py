...
def _replay_result(**kwargs: Any) -> ReplayResult:
...
def _manifest_drift_triage(message: str | None) -> ReplayTriage:
    clauses = _drift_message_clauses(message)
    first_change = clauses[0] if clauses else message
    drift_sources = [
        source_name
        for clause in clauses
        if (source_name := _context_source_name_from_clause(clause)) is not None
    ]
    if drift_sources or any("enriched context" in clause for clause in clauses):
        ...
        return ReplayTriage(
            ...
            headline=_context_source_headline(primary_source, first_change),
            first_relevant_change=first_change,
            drift_sources=drift_sources,
            recommended_inspection_path=_context_source_inspection_path(primary_source),
        )

    return ReplayTriage(
        severity="error",
        classification="manifest_drift",
        headline=_manifest_drift_headline(first_change),
        first_relevant_change=first_change,
        drift_sources=_manifest_drift_sources(first_change),
        recommended_inspection_path=_manifest_drift_inspection_path(first_change),
    )


def _drift_message_clauses(message: str | None) -> list[str]:
    if message is None:
        return []
    return [clause.strip() for clause in message.split(";") if clause.strip()]


def _context_source_name_from_clause(clause: str) -> str | None:
    markers = (
        "source drifted: ",
        "source added: ",
        "source missing: ",
...
def _manifest_drift_headline(first_change: str | None) -> str:
...
def _manifest_drift_sources(first_change: str | None) -> list[str]:
    if first_change is None:
        return []
    if first_change.startswith("prepared turn"):
...
def _manifest_drift_inspection_path(first_change: str | None) -> str:
    if first_change is None:
...
def _context_source_inspection_path(source_name: str | None) -> str:
...
def _mismatch_dimension(mismatch: str) -> str | None:
