"""Message builders for local handoff terminal commands."""
...
def handoff_message(
    ...
    return safe_handoff_commands_message(state)


def safe_handoff_commands_message(state: TerminalConversationState) -> str:
    session_id = str(state.header.session_id)
    cwd = state.header.cwd or "."
    return "\n".join(
        [
...
__all__ = ["handoff_message", "safe_handoff_commands_message"]
