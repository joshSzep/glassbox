...
from scripts.validate_package_contents import WHEEL_REQUIRED_FILES
...
def test_project_version_matches_package_and_v11_policy() -> None:
    project = _pyproject()["project"]
    version_policy = (REPO_ROOT / "docs" / "version-release-policy.md").read_text(
        encoding="utf-8"
    )
    v11_contract = (
        REPO_ROOT / "docs" / "v11-confidence-adoption-contract.md"
    ).read_text(encoding="utf-8")

    assert project["version"] == PACKAGE_VERSION
    assert glassbox.__version__ == PACKAGE_VERSION
    assert f"package target is `{PACKAGE_VERSION}`" in v11_contract
...
def test_build_targets_package_dashboard_static_assets() -> None:
    ...
    assert "/frontend/components/console/handoff" in hatch_config["sdist"]["include"]
    assert "/frontend/generated" in hatch_config["sdist"]["include"]
    assert "/frontend/stores/handoff-store*" in hatch_config["sdist"]["include"]
...
def test_distribution_content_validator_accepts_complete_wheel_and_sdist(
    ...
    dist_dir.mkdir()
    wheel_path = dist_dir / "glassbox-0.10.0-py3-none-any.whl"
    sdist_path = dist_dir / "glassbox-0.10.0.tar.gz"
...
def test_wheel_content_validator_reports_missing_metadata_and_assets(
    ...
    assert (
        "wheel missing required file: glassbox/runtime/handoff_package.py" in problems
    )
    assert "wheel missing required file: glassbox/cli/handoff_commands.py" in problems
    assert "wheel missing required file: glassbox/web/routes/handoffs.py" in problems
    assert (
...
def test_sdist_content_validator_reports_missing_docs_and_static_assets(
    ...
    sdist_path = tmp_path / "glassbox-0.10.0.tar.gz"
    ...
    assert (
        "sdist missing required file: docs/v11-confidence-adoption-contract.md"
        in problems
    )
    assert "sdist missing required file: docs/v11-residual-risk-audit.md" in problems
    assert "sdist missing required file: docs/tasks-v11.md" in problems
    assert (
    ...
    assert (
        "sdist missing required file: evals/fixtures/recommendation_cases.json"
        in problems
    )
    assert (
    ...
    assert (
        "sdist missing required file: "
        "evals/cases/changeset.review-loop-lifecycle.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/cases/changeset.in-session-review-ux.json" in problems
    )
    assert (
        "sdist missing required file: "
    ...
    assert (
        "sdist missing required file: "
        "evals/bundles/changeset.review-loop-lifecycle.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/bundles/changeset.in-session-review-ux.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/cases/changeset.lifecycle-rich-evidence.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/cases/changeset.response-linked-fixup-inventory.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/cases/changeset.skipped-advisory-evidence-posture.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/bundles/changeset.lifecycle-rich-evidence.json" in problems
    )
    assert (
        "sdist missing required file: "
        "evals/bundles/changeset.response-linked-fixup-inventory.json" in problems
    )
    assert (
        "sdist missing required file: "
    ...
    assert (
        "sdist missing required file: "
        "frontend/components/console/handoff/prepare-panel.tsx" in problems
    )
    assert (
        "sdist missing required file: frontend/stores/handoff-store-loaders.ts"
        in problems
    )
    assert "sdist missing required file: scripts/release_gate_runner.py" in problems
    assert (
        "sdist missing required file: scripts/v17_release_gate_stage_groups.py"
        in problems
    )
    assert (
    ...
    assert (
        "sdist missing required file: scripts/validate_v14_release_gate.py" in problems
    )
    assert (
        "sdist missing required file: scripts/v14_release_gate_helpers.py" in problems
    )
    assert "sdist missing required file: scripts/v14_release_gate_stages.py" in problems
    assert (
        "sdist missing required file: scripts/v14_release_gate_advisory.py" in problems
    )
    assert (
        "sdist missing required file: scripts/v14_release_gate_summary.py" in problems
    )
    assert (
...
def _write_wheel(path: Path) -> None:
    with zipfile.ZipFile(path, mode="w") as wheel:
        ...
        for wheel_file in WHEEL_REQUIRED_FILES:
            if wheel_file not in wheel.namelist():
                wheel.writestr(wheel_file, "")
        wheel.writestr(
            "glassbox-0.10.0.dist-info/METADATA",
            "Name: glassbox\nRequires-Dist: textual<7,>=6\n",
        )
        wheel.writestr(
            "glassbox-0.10.0.dist-info/entry_points.txt",
...
def _write_sdist(
    ...
    *,
    ...
    with tarfile.open(path, mode="w:gz") as sdist:
        _add_tar_text(sdist, "glassbox-0.10.0/README.md", "# Glassbox\n")
        _add_tar_text(sdist, "glassbox-0.10.0/LICENSE", "license\n")
        _add_tar_text(sdist, "glassbox-0.10.0/pyproject.toml", "[project]\n")
        if include_docs:
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/getting-started.md",
                "# Getting Started\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/operator-quickstart.md",
                "# Operator Quickstart\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/daily-workflow-quickstart.md",
                "# Daily Workflow Quickstart\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/providers.md",
                "# Provider Setup\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/version-release-policy.md",
                "# Version And Release Naming Policy\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/v9-public-baseline.md",
                "# Glassbox v9 Public Baseline\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/v9-vocabulary.md",
            ...
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/release-packaging.md",
                "# Release Packaging\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/v7-release-candidate.md",
                "# v7 Release Candidate\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/v7-release-gate.md",
                "# v7 Release Gate\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-v7-release-validation.md",
                "# v7 Manual Release Validation\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-v8-release-validation.md",
                "# v8 Manual Release Validation\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-v9-release-validation.md",
                "# v9 Manual Release Validation\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/workspace-profiles.md",
                "# Workspace Profiles\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-qa-evidence-v7.md",
                "# v7 Manual QA Evidence Archive\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-qa-evidence-v8.md",
                "# v8 Manual QA Evidence Archive\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/docs/manual-qa-evidence-v9.md",
            ...
            for doc_path in (
                ...
                _add_tar_text(sdist, f"glassbox-0.10.0/{doc_path}", "# v8\n")
            for eval_path in (
                "evals/profiles.json",
                "evals/coverage.json",
                "evals/impact.json",
                ...
                "evals/cases/changeset.in-session-review-ux.json",
                "evals/cases/changeset.lifecycle-rich-evidence.json",
                "evals/cases/changeset.response-linked-fixup-inventory.json",
                "evals/cases/changeset.review-loop-lifecycle.json",
                "evals/cases/changeset.reviewable-lifecycle.json",
                "evals/cases/changeset.skipped-advisory-evidence-posture.json",
                ...
                "evals/bundles/changeset.in-session-review-ux.json",
                "evals/bundles/changeset.lifecycle-rich-evidence.json",
                "evals/bundles/changeset.response-linked-fixup-inventory.json",
                "evals/bundles/changeset.review-loop-lifecycle.json",
                "evals/bundles/changeset.reviewable-lifecycle.json",
                "evals/bundles/changeset.skipped-advisory-evidence-posture.json",
                ...
                _add_tar_text(sdist, f"glassbox-0.10.0/{eval_path}", "{}\n")
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/frontend/generated/openapi.json",
                "{}\n",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/frontend/generated/api-types.ts",
            ...
            for frontend_path in (
                ...
                "frontend/components/console/handoff/shared.tsx",
                ...
                _add_tar_text(sdist, f"glassbox-0.10.0/{frontend_path}", "\n")
            for script_path in (
        ...
        if include_static_assets:
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/src/glassbox/web/static_next/index.html",
                "<html></html>",
            )
            _add_tar_text(
                sdist,
                "glassbox-0.10.0/src/glassbox/web/static_next/_next/static/chunks/app.js",
