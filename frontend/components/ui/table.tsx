...
>(({ className, ...props }, ref) => (
  <thead className={cn("bg-muted/70 [&_tr]:border-b", className)} ref={ref} {...props} />
));
...
const TableFooter = React.forwardRef<
  ...
  React.HTMLAttributes<HTMLTableSectionElement>
>(({ className, ...props }, ref) => (
  <tfoot className={cn("border-t bg-muted font-medium", className)} ref={ref} {...props} />
));
...
>(({ className, ...props }, ref) => (
  <th
    className={cn(
      "h-9 px-3 text-left align-middle text-xs font-semibold uppercase tracking-normal text-muted-foreground [&:has([role=checkbox])]:pr-0",
      className,
    )}
    ref={ref}
    {...props}
