        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              onClick={() => {
                if (compare.sessionId !== null) {
                  onOpenSession?.(compare.sessionId);
                }
              }}
              size="xs"
              type="button"
...
function buildCompareDifferences(
  ...
  const currentWorkingSet = current.runtimeContext?.working_set?.items ?? [];
  const compareWorkingSet = compare.runtimeContext?.working_set?.items ?? [];
  const workingSetDelta = compareStringSets(
    currentWorkingSet.map((item) => item.subject),
    compareWorkingSet.map((item) => item.subject),
  );
  const currentRuntimePaths = current.runtimeContext?.repository_context.high_signal_paths ?? [];
  const compareRuntimePaths = compare.runtimeContext?.repository_context.high_signal_paths ?? [];
  const runtimePathDelta = compareStringSets(currentRuntimePaths, compareRuntimePaths);
  ...
  return [
    ...
    },
    {
      ...
      value: `${current.transcript.length} current · ${compare.transcript.length} compared · delta ${
        current.transcript.length - compare.transcript.length
      }`,
    },
    {
