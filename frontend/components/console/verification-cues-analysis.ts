...
import type { EvidenceCue, WorkingSetItem } from "@/components/console/verification-cues-sections";
import type { DashboardState } from "@/state/session-state";

export type ArtifactSummary = NonNullable<
  NonNullable<NonNullable<DashboardState["runtimeContext"]>["artifact_context"]>["summaries"]
>[number];

export type VerificationCueAnalysis = {
  advisoryArtifacts: ArtifactSummary[];
  artifactSummaries: ArtifactSummary[];
  blockingArtifacts: ArtifactSummary[];
  evidenceCues: EvidenceCue[];
  inheritedWorkingSetCount: number;
  missingArtifactCount: number;
  policyCueCount: number;
  verifiedArtifacts: ArtifactSummary[];
  workingSetItems: WorkingSetItem[];
};

export function deriveVerificationCueAnalysis(data: DashboardState): VerificationCueAnalysis {
...
export function buildEvidenceCues(
  data: DashboardState,
...
export function countPolicyCues(data: DashboardState): number {
  return Math.max(
...
export function isBlockingCue(artifact: ArtifactSummary): boolean {
...
export function isAdvisoryCue(artifact: ArtifactSummary): boolean {
  return isProviderEvidence(artifact) || isDriftCue(artifact);
}

export function artifactBadgeVariant(artifact: ArtifactSummary) {
...
export function artifactCueLabel(artifact: ArtifactSummary): string {
  if (isBlockingCue(artifact)) {
...
export function matchingTargetPaths(
  artifacts: ArtifactSummary[],
  workingSetItems: WorkingSetItem[],
): string[] {
