      ...
      <DataList density="compact">
        {cues.map((cue) => (
          <DataListItem key={cue.label}>
            ...
            <code className="block overflow-x-auto rounded-md border border-border/70 bg-surface-raised px-2 py-1 text-xs text-muted-foreground">
              {cue.command}
            </code>
          </DataListItem>
        ))}
...
function recoveryCues(data: DashboardState): RecoveryCue[] {
  ...
  return [
    {
      command: "uv run glassbox daemon status --cwd .",
    ...
    {
      command: "uv run glassbox projection check --all --cwd .",
      detail:
        projectionAlerts > 0
    ...
    {
      command: "uv run glassbox job list --state failed --cwd .",
      detail:
    ...
    {
      command: "uv run glassbox repo index status --cwd .",
      detail:
    ...
    {
      command: "uv run glassbox artifacts inspect --json --cwd .",
      detail:
    ...
    {
      command: "uv run glassbox memory list --state invalidated --cwd .",
    ...
    {
      command: "uv run glassbox provider canary evidence --cwd .",
      detail:
        "Review retained provider evidence freshness and advisory canary status without treating it as release authority.",
