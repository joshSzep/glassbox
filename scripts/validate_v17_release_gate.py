...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.dry_run:
        ...
        _record_v17_provider_evidence(
            summary,
            evidence_dir,
            include=args.include_provider_canaries,
            dry_run=True,
        )
        _record_v17_advisory_evidence(summary, evidence_dir)
    ...
    if wheel_path is None:
        ...
        _write_evidence_summary(evidence_dir, summary)
        _print_summary(summary)
        print("V17 release gate failed: built wheel not found", file=sys.stderr)
        return 1
...
def _record_v17_provider_evidence(
    ...
    *,
    ...
    v17_helpers.record_provider_evidence(
        summary,
        evidence_dir,
        include=include,
        dry_run=dry_run,
...
def _record_v17_advisory_evidence(
    summary: dict[str, Any],
    evidence_dir: Path,
) -> None:
    v17_helpers.record_advisory_evidence(summary, evidence_dir)
...
def _run_stage(summary: dict[str, Any], stage: GateStage) -> int:
    ...
    if result.returncode != 0:
        print(f"\nV17 release gate failed: {stage.label}", file=sys.stderr)
...
def _record_planned_stages(
    ...
    v17_helpers.record_planned_stages(summary, stages)


def _record_installed_wheel_plan(summary: dict[str, Any]) -> None:
    v17_helpers.record_installed_wheel_plan(summary)
...
def _resolve_evidence_dir(requested: Path | None) -> Path:
    return v17_helpers.resolve_evidence_dir(requested)
...
def _new_evidence_summary(
    ...
    *,
    ...
    return v17_helpers.new_evidence_summary(
        evidence_dir,
        include_provider_canaries=include_provider_canaries,
        dry_run=dry_run,
    )
...
def _finish_summary(summary: dict[str, Any], status: str) -> None:
    v17_helpers.finish_summary(summary, status)


def _write_evidence_summary(evidence_dir: Path, summary: dict[str, Any]) -> Path:
    return v17_helpers.write_evidence_summary(evidence_dir, summary)


def _print_summary(summary: dict[str, Any]) -> None:
    v17_helpers.print_summary(summary)


def _now_iso() -> str:
    return v17_helpers.now_iso()


def _now_stamp() -> str:
    return v17_helpers.now_stamp()
