"""Run the v14 review-loop maturity release gate scaffold."""
...
def build_gate_stages(evidence_dir: Path | None = None) -> list[GateStage]:
    ...
    return v14_helpers.build_gate_stages(evidence_dir)


def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the Glassbox v14 review-loop maturity release gate.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if args.dry_run:
        ...
        _record_v14_provider_evidence(
            summary,
            evidence_dir,
            include=args.include_provider_canaries,
            dry_run=True,
        )
        _record_v14_advisory_ux_evidence(summary, evidence_dir)
    ...
    if wheel_path is None:
        ...
        _write_evidence_summary(evidence_dir, summary)
        _print_summary(summary)
        print("V14 release gate failed: built wheel not found", file=sys.stderr)
        return 1
...
def _record_v14_provider_evidence(
...
def _record_v14_advisory_ux_evidence(
    summary: dict[str, Any],
    evidence_dir: Path,
) -> None:
    v14_helpers.record_v14_advisory_ux_evidence(summary, evidence_dir)
...
def _run_stage(summary: dict[str, Any], stage: GateStage) -> int:
    ...
    if result.returncode != 0:
        print(f"\nV14 release gate failed: {stage.label}", file=sys.stderr)
...
def _resolve_evidence_dir(requested: Path | None) -> Path:
    return v14_helpers.resolve_evidence_dir(requested)
...
def _new_evidence_summary(
    ...
    *,
    ...
    return v14_helpers.new_evidence_summary(
        evidence_dir,
        include_provider_canaries=include_provider_canaries,
        dry_run=dry_run,
    )
...
def _finish_summary(summary: dict[str, Any], status: str) -> None:
    v14_helpers.finish_summary(summary, status)
...
def _write_evidence_summary(evidence_dir: Path, summary: dict[str, Any]) -> Path:
    ...
    print(f"\nV14 release evidence written to {summary_path}")
    return summary_path
