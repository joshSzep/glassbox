...
from scripts import v13_release_gate_helpers as v13_helpers  # noqa: E402
...
def build_gate_stages(evidence_dir: Path | None = None) -> list[GateStage]:
    ...
    return v13_helpers.build_gate_stages(evidence_dir)
...
def _record_v13_provider_evidence(
    ...
    *,
    ...
    v13_helpers.record_v13_provider_evidence(
        summary,
        evidence_dir,
        include=include,
        dry_run=dry_run,
...
def _record_v13_browser_accessibility_evidence(
    ...
    v13_helpers.record_v13_browser_accessibility_evidence(summary, evidence_dir)
...
def _record_planned_stages(
    ...
    v13_helpers.record_planned_stages(summary, stages)


def _record_installed_wheel_plan(summary: dict[str, Any]) -> None:
    v13_helpers.record_installed_wheel_plan(summary)
...
def _resolve_evidence_dir(requested: Path | None) -> Path:
    return v13_helpers.resolve_evidence_dir(requested)


def _eval_evidence_dir(evidence_dir: Path) -> Path:
    return v13_helpers.eval_evidence_dir(evidence_dir)
...
def _new_evidence_summary(
    ...
    *,
    ...
    return v13_helpers.new_evidence_summary(
        evidence_dir,
        include_provider_canaries=include_provider_canaries,
        dry_run=dry_run,
    )
...
def _finish_summary(summary: dict[str, Any], status: str) -> None:
    v13_helpers.finish_summary(summary, status)
