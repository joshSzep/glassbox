"""Run the v12 reviewable-change release gate scaffold."""
...
from scripts.validate_v11_release_gate import (  # noqa: E402
    build_gate_stages as build_v11_gate_stages,
)
from scripts.validate_v11_release_gate import (  # noqa: E402
...
def build_gate_stages(evidence_dir: Path | None = None) -> list[GateStage]:
    """Return the deterministic blocking stages for the v12 gate."""

    resolved_evidence_dir = evidence_dir or Path(".glassbox/releases/v12-gate")
    eval_output_dir = _eval_evidence_dir(resolved_evidence_dir)
    return [
        *build_v11_gate_stages(resolved_evidence_dir),
        GateStage(
            ...
            (
                ...
                "release-candidate",
                "--output-dir",
                str(eval_output_dir / "v12-release-signoff"),
                "--cwd",
                ".",
            ),
        ),
        GateStage(
            ...
            (
                ...
                "release-candidate",
                "--output-dir",
                str(eval_output_dir / "v12-reviewable-change-release"),
                "--refresh-output-dir",
                "--cwd",
                ".",
            ),
        ...
        GateStage(
            ...
            (
                ...
                "--output-dir",
                str(eval_output_dir / "v12-changeset-lifecycle-smoke"),
                "--refresh-output-dir",
                "--cwd",
                ".",
            ),
...
def main(argv: Sequence[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run the Glassbox v12 reviewable-change release gate.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
    ...
    if wheel_path is None:
        ...
        _write_evidence_summary(evidence_dir, summary)
        _print_summary(summary)
        print("V12 release gate failed: built wheel not found", file=sys.stderr)
        return 1
...
def _record_v12_provider_evidence(
    ...
    *,
    ...
    if summary["advisory"]:
        ...
        if latest.get("label") == "v11 advisory provider evidence":
            latest["label"] = "v12 advisory provider evidence"
...
def _run_stage(summary: dict[str, Any], stage: GateStage) -> int:
    ...
    if result.returncode != 0:
        print(f"\nV12 release gate failed: {stage.label}", file=sys.stderr)
...
def _resolve_evidence_dir(requested: Path | None) -> Path:
    if requested is not None:
        return requested
    return DEFAULT_EVIDENCE_ROOT / f"{_now_stamp()}-v12-gate"
...
def _new_evidence_summary(
    ...
    *,
    ...
    summary["gate"] = "v12-release"
    summary["artifacts"].update(
        {
            "v12_task_graph": "docs/tasks-v12.md",
            "v12_reviewable_change_contract": "docs/v12-reviewable-change-contract.md",
            "v12_change_lifecycle_audit": "docs/v12-change-lifecycle-audit.md",
            "v12_release_gate": "docs/v12-release-gate.md",
            "v12_eval_cases": "evals/README.md",
            "v12_replay_evals": "docs/replay-evals.md",
        }
    )
...
def _finish_summary(summary: dict[str, Any], status: str) -> None:
    ...
    summary["next_actions"] = [
        action.replace("v11", "v12") for action in summary["next_actions"]
    ]
...
def _write_evidence_summary(evidence_dir: Path, summary: dict[str, Any]) -> Path:
    ...
    print(f"\nV12 release evidence written to {summary_path}")
    return summary_path
